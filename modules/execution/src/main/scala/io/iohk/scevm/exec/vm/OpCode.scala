...
import io.iohk.scevm.domain.UInt256.{Zero, _}
import io.iohk.scevm.domain.{Account, Address, TransactionLogEntry, UInt256}
import io.iohk.scevm.exec.config.BlockchainConfigForEvm.EthForks.EthFork
import io.iohk.scevm.exec.config.BlockchainConfigForEvm.{EthForks, isEip2929Enabled}
import io.iohk.scevm.exec.config.FeeSchedule
...
object OpCodes {
  ...
  val IstanbulOpcodes: List[OpCode] =
    List(CHAINID, SELFBALANCE) ++ ConstantinopleOpCodes

  val LondonOpcodes: List[OpCode] =
    List(BASEFEE) ++ IstanbulOpcodes
}
...
object OpCode {
  ...
  def addressAccessCost[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S], address: Address)(
  ...
  def storageAccessCost[W <: WorldState[W, S], S <: Storage[S]](
  ...
  def accessCost[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S], isWarm: Boolean)(
      preGasFn: FeeSchedule => BigInt,
      postColdGasFn: FeeSchedule => BigInt,
      postWarmGasFn: FeeSchedule => BigInt
  ): BigInt = {
    val currentBlockNumber = state.env.blockContext.number
    val ethFork            = state.config.blockchainConfig.ethForkForBlockNumber(currentBlockNumber)
    val eip2929Enabled     = isEip2929Enabled(ethFork)
    if (eip2929Enabled) {
      if (isWarm)
...
abstract class OpCode(val code: Byte, val delta: Int, val alpha: Int, val baseGasFn: FeeSchedule => BigInt)
  ...
  def execute[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] =
  ...
  def calcGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt =
    baseGas(state) + varGas(state)

  protected def baseGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = baseGasFn(
    state.config.feeSchedule
  )

  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt

  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S]

  protected def availableInContext[W <: WorldState[W, S], S <: Storage[S]]: ProgramState[W, S] => Boolean = _ => true
...
trait AddrAccessGas { self: OpCode =>
  ...
  override protected def baseGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt =
    OpCode.addressAccessCost(state, address(state))(baseGasFn, coldGasFn, warmGasFn)

  protected def address[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): Address
}
...
trait StorageAccessGas { self: OpCode =>
  ...
  protected def addressAndKey[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S]
  ): (Address, BigInt)
}

sealed trait ConstGas { self: OpCode =>
  @nowarn("msg=parameter value state in method varGas is never used")
  override protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = 0
}
...
sealed abstract class BinaryOp(code: Int, baseGasFn: FeeSchedule => BigInt)(val f: (UInt256, UInt256) => UInt256)
  ...
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((a, b), stack1) = state.stack.pop2
    val res              = f(a, b)
    val stack2           = stack1.push(res)
    state.withStack(stack2).step()
  }
...
sealed abstract class TernaryOp(code: Int, baseGasFn: FeeSchedule => BigInt)(
  ...
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((a, b, c), stack1) = state.stack.pop3
    val res                 = f(a, b, c)
    val stack2              = stack1.push(res)
    state.withStack(stack2).step()
  }
...
sealed abstract class ConstOp(code: Int)(
    val f: ProgramState[_ <: WorldState[_, _ <: Storage[_]], _ <: Storage[_]] => UInt256
...
sealed abstract class ShiftingOp(code: Int, f: (UInt256, UInt256) => UInt256)
    ...
    with ConstGas {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((shift: UInt256, value: UInt256), remainingStack) = state.stack.pop2
...
case object EXP extends BinaryOp(0x0a, _.G_exp)(_ ** _) {
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((_, m: UInt256), _) = state.stack.pop2
...
case object SAR extends OpCode(0x1d, 2, 1, _.G_verylow) with ConstGas {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((shift, value), remainingStack) = state.stack.pop2
...
case object SHA3 extends OpCode(0x20, 2, 1, _.G_sha3) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((offset, size), stack1) = state.stack.pop2
  ...
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((offset, size), _) = state.stack.pop2
...
case object CALLDATACOPY extends OpCode(0x37, 3, 0, _.G_verylow) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((memOffset, dataOffset, size), stack1) = state.stack.pop3
  ...
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((offset, _, size), _) = state.stack.pop3
    val memCost                = state.config.calcMemCost(state.memory.size, offset, size)
...
case object CODECOPY extends OpCode(0x39, 3, 0, _.G_verylow) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((memOffset, codeOffset, size), stack1) = state.stack.pop3
...
/** According to eip-1559 this is `priority_fee_per_gas + block.base_fee_per_gas`
  * we will assume `block.base_fee_per_gas` is 0, so no need to add it
  */
case object GASPRICE
    extends ConstOp(0x3a)(state => state.env.priorityFeePerGas + state.env.evmConfig.ethCompatibility.blockBaseFee)

// EIP-3198
case object BASEFEE extends ConstOp(0x48)(_.env.evmConfig.ethCompatibility.blockBaseFee)
...
case object EXTCODECOPY extends OpCode(0x3c, 4, 0, _.G_extcode) with AddrAccessGas {
  ...
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((address, memOffset, codeOffset, size), stack1) = state.stack.pop4
  ...
  override protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((_, memOffset, _, size), _) = state.stack.pop4
  ...
  protected def address[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): Address = {
    val ((accountAddress, _, _, _), _) = state.stack.pop4
    Address(accountAddress)
  }
}
...
case object BLOCKHASH extends OpCode(0x40, 1, 1, _.G_blockhash) with ConstGas {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    ...
    val headerBlockNumber = state.env.blockContext.number.value
    val outOfLimits       = headerBlockNumber - blockNumber > 256 || blockNumber >= headerBlockNumber
...
case object COINBASE extends ConstOp(0x41)(s => UInt256(s.env.blockContext.coinbase.bytes))

case object TIMESTAMP extends ConstOp(0x42)(s => UInt256(s.env.blockContext.unixTimestamp.toSeconds))

case object NUMBER extends ConstOp(0x43)(s => UInt256(s.env.blockContext.number.value))

// There is no difficulty concept in the case of OBFT, so this opcode is just there for the compatibility
// It just returns a constant value defined in the configuration
case object DIFFICULTY extends ConstOp(0x44)(_.env.evmConfig.ethCompatibility.difficulty)

case object GASLIMIT extends ConstOp(0x45)(s => UInt256(s.env.blockContext.gasLimit))
...
case object MSTORE extends OpCode(0x52, 2, 0, _.G_verylow) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((offset, value), stack1) = state.stack.pop2
...
case object SSTORE extends OpCode(0x55, 2, 0, _.G_zero) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    ...
    val eip2200Enabled = isEip2200Enabled(ethFork)
    val eip1283Enabled = isEip1283Enabled(ethFork)

    val ((offset, newValue), stack1) = state.stack.pop2
  ...
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((offset, newValue), _) = state.stack.pop2
    val currentValue            = state.storage.load(offset)
  ...
  override protected def availableInContext[W <: WorldState[W, S], S <: Storage[S]]: ProgramState[W, S] => Boolean =
    !_.staticCtx
  ...
  private def isEip2200Enabled(ethFork: EthFork): Boolean =
    ethFork >= EthForks.Istanbul
}
...
case object JUMPI extends OpCode(0x57, 2, 0, _.G_high) with ConstGas {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((pos, cond), stack1) = state.stack.pop2
    val dest                  = pos.toInt // fail with InvalidJump if conversion to Int is lossy
...
sealed abstract class LogOp(code: Int, val topicNb: Int) extends OpCode(code, topicNb + 2, 0, _.G_log) {

  def this(code: Int) = this(code, code - 0xa0)

  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((offset, size), stack1) = state.stack.pop2
    val (topics, stack2)         = stack1.pop(topicNb)
    val (data, memory)           = state.memory.load(offset, size)
    val logEntry                 = TransactionLogEntry(state.env.ownerAddr, topics.map(_.bytes), data)

    state.withStack(stack2).withMemory(memory).withLog(logEntry).step()
  }

  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((offset, size), _) = state.stack.pop2
    val memCost             = state.config.calcMemCost(state.memory.size, offset, size)
    val logCost             = state.config.feeSchedule.G_logdata * size + topicNb * state.config.feeSchedule.G_logtopic
    memCost + logCost
  }
...
sealed abstract class CreateOp(code: Int, delta: Int) extends OpCode(code, delta, 1, _.G_create) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val ((endowment, inOffset, inSize), stack1) = state.stack.pop3
    ...
    // intrinsicGas will not be used in this method, no need to calculate it
    val intrinsicGas = BigInt(0)

    val context: ProgramContext[W, S] = ProgramContext(
      callerAddr = state.env.ownerAddr,
      originAddr = state.env.originAddr,
      recipientAddr = None,
      maxPriorityFeePerGas = state.env.priorityFeePerGas,
      startGas = startGas,
      ...
      endowment = endowment,
      doTransfer = true,
      blockContext = state.env.blockContext,
    ...
    val ((result, newAddress), stack2) = this match {
      case CREATE => (state.create(context, None), stack1)
      case CREATE2 =>
        val (salt, stack2) = stack1.pop
        (state.create(context, Some(salt)), stack2)
    }
...
case object CREATE extends CreateOp(0xf0, 3) {
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val ((_, inOffset, inSize), _) = state.stack.pop3
...
sealed abstract class CallOp(code: Int, delta: Int, alpha: Int) extends OpCode(code, delta, alpha, _.G_zero) {
  import CallOp.Params
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    val (params @ Params(_, to, callValue, inOffset, inSize, outOffset, outSize), stack1) = getParams(state)
    ...
    // intrinsicGas will not be used in this methods, no need to calculate it
    val intrinsicGas = BigInt(0)
    ...
    val result = state.call(context, owner)
  ...
  protected def varGas[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): BigInt = {
    val (Params(gas, to, callValue, inOffset, inSize, outOffset, outSize), _) = getParams(state)
  ...
  protected def calcMemCost[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S],
      inOffset: UInt256,
      inSize: UInt256,
      outOffset: UInt256,
  ...
  /** Returns (gas, to, callValue, inOffset, inSize, outOffset, outSize) with the resulting stack */
  private def getParams[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S]
  ): (Params, Stack) = {
    val ((gas, to), stack1)                              = state.stack.pop2
    val (value, stack2)                                  = if (this == DELEGATECALL || this == STATICCALL) (state.env.value, stack1) else stack1.pop
    val ((inOffset, inSize, outOffset, outSize), stack3) = stack2.pop4
    Params(gas, to, value, inOffset, inSize, outOffset, outSize) -> stack3
  }

  protected def calcStartGas[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S],
      params: Params,
      endowment: UInt256
  ): BigInt = {
    val Params(gas, to, _, inOffset, inSize, outOffset, outSize) = params
    val memCost                                                  = calcMemCost(state, inOffset, inSize, outOffset, outSize)
  ...
  private def gasCap[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S],
      g: BigInt,
      consumedGas: BigInt
  ): BigInt =
  ...
  private def gasExtra[W <: WorldState[W, S], S <: Storage[S]](
      state: ProgramState[W, S],
      endowment: UInt256,
      to: Address
  ): BigInt = {
...
case object CALL extends CallOp(0xf1, 7, 1) {
  override protected def availableInContext[W <: WorldState[W, S], S <: Storage[S]]: ProgramState[W, S] => Boolean =
    state =>
      !state.staticCtx || {
        val ((_, _, callValue), _) = state.stack.pop3
        callValue.isZero
      }
}
...
case object SELFDESTRUCT extends OpCode(0xff, 1, 0, _.G_selfdestruct) {
  protected def exec[W <: WorldState[W, S], S <: Storage[S]](state: ProgramState[W, S]): ProgramState[W, S] = {
    ...
    state.selfDestruct(context, state)
  }
