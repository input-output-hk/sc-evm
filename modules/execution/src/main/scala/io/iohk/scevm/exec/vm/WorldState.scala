...
/** This is a single entry point to all VM interactions with the persisted state. Implementations are meant to be
  ...
  */
trait WorldState[WS <: WorldState[WS, S], S <: Storage[S]] { self: WS =>
  ...
  protected def accountStartNonce: Nonce = Nonce.Zero
  ...
  /** Returns the account or an empty account if it does not exist. */
  protected def getGuaranteedAccount(address: Address): Account =
    getAccount(address).getOrElse(getEmptyAccount)
  ...
  def initialiseAccount(newAddress: Address): WS = {
    ...
    val accountWithCorrectNonce =
      if (!noEmptyAccounts)
        ...
        newAccount.copy(nonce = accountStartNonce.increaseOne)
  ...
  def createAddress(creatorAddr: Address): Address = {
    ...
    val hash           = kec256(rlp.encode(RLPList(creatorAddr.bytes, rlp.decode[Nonce](creatorAccount.nonce.decreaseOne))))
    Address(hash)
  }
  ...
  def increaseNonce(address: Address): WS = {
    val account = getGuaranteedAccount(address).increaseNonceOnce
