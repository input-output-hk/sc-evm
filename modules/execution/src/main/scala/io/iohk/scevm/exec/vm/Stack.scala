...
class Stack private (private val underlying: Vector[UInt256], val maxSize: Int) {
  ...
  def pop2: ((UInt256, UInt256), Stack) = {
    val (seq, stack) = pop(2)
    seq match {
      case Seq(a, b) => ((a, b), stack)
    }
  }

  @nowarn("msg=match may not be exhaustive")
  def pop3: ((UInt256, UInt256, UInt256), Stack) = {
    val (seq, stack) = pop(3)
    seq match {
      case Seq(a, b, c) => ((a, b, c), stack)
    }
  }

  @nowarn("msg=match may not be exhaustive")
  def pop4: ((UInt256, UInt256, UInt256, UInt256), Stack) = {
    val (seq, stack) = pop(4)
    seq match {
      case Seq(a, b, c, d) => ((a, b, c, d), stack)
    }
  }
