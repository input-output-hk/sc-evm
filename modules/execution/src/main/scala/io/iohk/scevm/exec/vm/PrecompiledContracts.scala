...
object PrecompiledContracts {
  ...
  val byzantiumContracts: Map[Address, PrecompiledContract] = contracts ++ Map(
    ModExpAddr       -> ModExp,
  ...
  val istanbulContracts: Map[Address, PrecompiledContract] = byzantiumContracts ++ Map(
  ...
  def getContracts(context: ProgramContext[_, _]): Map[Address, PrecompiledContract] = {
    val ethFork = context.evmConfig.blockchainConfig.ethForkForBlockNumber(context.blockContext.number)

    if (ethFork >= EthForks.Istanbul) {
      istanbulContracts
    } else if (ethFork >= EthForks.Byzantium) {
      byzantiumContracts
    } else
      contracts
  }
  ...
  sealed trait PrecompiledContract {
    ...
    protected def gas(inputData: ByteString, ethFork: EthFork): BigInt

    def run[W <: WorldState[W, S], S <: Storage[S]](context: ProgramContext[W, S]): ProgramResult[W, S] = {

      val ethFork = context.evmConfig.blockchainConfig.ethForkForBlockNumber(context.blockContext.number)

      val g = gas(context.inputData, ethFork)
      ...
      ProgramResult(
        ...
        Nil,
        0,
        Duration(deltaTime, NANOSECONDS),
        error,
        None,
        Set.empty,
        Set.empty
  ...
  object EllipticCurveRecovery extends PrecompiledContract {
    ...
    def gas(inputData: ByteString, ethFork: EthFork): BigInt = 3000
  ...
  object Sha256 extends PrecompiledContract {
    ...
    def gas(inputData: ByteString, ethFork: EthFork): BigInt =
  ...
  object Bn128Pairing extends PrecompiledContract {
    ...
    def gas(inputData: ByteString, ethFork: EthFork): BigInt = {
      ...
      if (ethFork >= EthForks.Istanbul) { // https://eips.ethereum.org/EIPS/eip-1108
        34000 * k + 45000
      } else {
        80000 * k + 100000
      }
