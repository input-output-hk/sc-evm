...
import io.iohk.scevm.domain.UInt256._
import io.iohk.scevm.domain.{Account, Address, BlockNumber, TransactionLogEntry, UInt256}
import io.iohk.scevm.exec.config.EvmConfig
import io.iohk.scevm.exec.vm.Generators._
...
class OpCodeFunSpec extends AnyFunSuite with OpCodeTesting with Matchers with ScalaCheckPropertyChecks {
  ...
  override val config: EvmConfig = EvmConfig.IstanbulConfigBuilder(blockchainConfig)
  ...
  test(ternaryOps: _*) { op =>
    forAll(getProgramStateGen()) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((a, b, c), _) = stateIn.stack.pop3
        val (result, _)    = stateOut.stack.pop
  ...
  test(constOps.diff(Seq(MSIZE, DIFFICULTY)): _*) { op =>
    forAll(getProgramStateGen()) { stateIn =>
      val stateOut = executeOp(op, stateIn)
  ...
  test(DIFFICULTY) { op =>
    val baseFee = 0
    val evmConfigGen =
      Gen.posNum[Long].map(difficulty => config.copy(ethCompatibility = EthCompatibilityConfig(difficulty, baseFee)))
    forAll(getProgramStateGen(evmConfigGen = evmConfigGen)) { stateIn =>
      val stateOut = executeOp(op, stateIn)
      withStackVerification(op, stateIn, stateOut) {
        val (difficulty, _)  = stateOut.stack.pop
        val ethCompatibility = stateIn.env.evmConfig.ethCompatibility
        difficulty shouldEqual ethCompatibility.difficulty
      }
    }
  }
  ...
  test(BASEFEE) { op =>
    val difficulty = 1
    val evmConfigGen =
      Gen.posNum[Long].map(baseFee => config.copy(ethCompatibility = EthCompatibilityConfig(difficulty, baseFee)))
    forAll(getProgramStateGen(evmConfigGen = evmConfigGen)) { stateIn =>
      val stateOut = executeOp(op, stateIn)
      withStackVerification(op, stateIn, stateOut) {
        val (baseFee, _)     = stateOut.stack.pop
        val ethCompatibility = stateIn.env.evmConfig.ethCompatibility
        baseFee shouldEqual ethCompatibility.blockBaseFee
      }
    }
  }
  ...
  test(SHA3) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((offset, size), _) = stateIn.stack.pop2
  ...
  test(BALANCE) { op =>
    forAll(getProgramStateGen(), getUInt256Gen()) { (stateIn, accountBalance) =>
      ...
      val address = Address(addr.mod(UInt256(BigInt(2).pow(160))))
      val world1  = stateIn.world.saveAccount(address, account)
      ...
      withStackVerification(op, stateInWithAccount, stateOutWithAccount) {
        ...
        val expectedState = stateInWithAccount
          .addAccessedAddress(address)
          .withStack(stack2)
          .step()
        stateOutWithAccount shouldEqual expectedState
      }
    }
  }
  ...
  test(CALLDATACOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((memOffset, dataOffset, size), _) = stateIn.stack.pop3
  ...
  test(CODECOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((memOffset, codeOffset, size), _) = stateIn.stack.pop3
  ...
  test(EXTCODECOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((addr, memOffset, codeOffset, size), _) = stateIn.stack.pop4
  ...
  test(BLOCKHASH) { op =>
    val stateGen: Gen[PS] = for {
      stateIn <- getProgramStateGen(
                   ...
                   blockNumberGen = getBlockNumberGen(max = BlockNumber(512))
                 )
    } yield stateIn
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        ...
        val withinLimits =
          (stateIn.env.blockContext.number.value - blockHeaderNumber.toBigInt) <= 256 &&
            blockHeaderNumber.toBigInt < stateIn.env.blockContext.number.value
  ...
  test(MSTORE) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((offset, value), _) = stateIn.stack.pop2
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        ...
        val expectedState = stateIn
          ...
          .addAccessedAddress(stateIn.ownAddress)
          .addAccessedStorageKey(stateIn.ownAddress, offset)
          .step()
        stateOut shouldEqual expectedState
      }
  ...
  test(JUMP) { op =>
    ...
    val List(validDest, insidePush) =
      code.indices.toList.filter(i => code(i) == JUMPDEST.code): @nowarn("msg=exhaustive")
  ...
  test(JUMPI) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((dest, cond), _) = stateIn.stack.pop2
        val expectedState =
          if (cond.isZero)
  ...
  test(logOps: _*) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val (Seq(offset, size, topics @ _*), stack1) = stateIn.stack.pop(op.delta): @nowarn("msg=exhaustive")
        val (data, mem1)                             = stateIn.memory.load(offset, size)
        val logEntry                                 = TransactionLogEntry(stateIn.env.ownerAddr, topics.map(_.bytes), data)
        val expectedState                            = stateIn.withStack(stack1).withMemory(mem1).withLog(logEntry).step()

        logEntry.logTopics.size shouldEqual op.topicNb
        stateOut shouldEqual expectedState
      }
    }
  }
  ...
  test(RETURNDATACOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        val ((memOffset, offset, size), _) = stateIn.stack.pop3
    ...
    forAll(stateGen) { stateIn =>
      ...
      withStackVerification(op, stateIn, stateOut) {
        ...
        val expectedState = stateIn
          ...
          .addAccessedAddresses(Set(Address(refundAddr), stateIn.env.ownerAddr))
          .halt
        stateOut shouldEqual expectedState
      }
    }
