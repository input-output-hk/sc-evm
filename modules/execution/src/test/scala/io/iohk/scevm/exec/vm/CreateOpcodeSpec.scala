...
import io.iohk.scevm.domain.{Account, Address, BlockContext, Nonce, UInt256}
...
class CreateOpcodeSpec extends AnyWordSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  object fxt {
    val fakeBlockContext: BlockContext =
      ValidBlock.blockContext.copy(number = blockchainConfig.constantinopleBlockNumber.previous)
  ...
  def commonBehaviour(opcode: CreateOp): Unit = {
    ...
    "initialization code executes normally" should {
      ...
      "update sender (creator) account" in {
        ...
        updatedCreator.nonce shouldEqual initialCreator.nonce.increaseOne
      }
    ...
    "initialization code fails" should {
      ...
      "not modify world state except for the creator's nonce" in {
        ...
        val expectedWorld =
          context.world.saveAccount(
            fxt.creatorAddr,
            creatorsAccount.copy(nonce = creatorsAccount.nonce.increaseOne)
          )
    ...
    "initialization code runs normally but there's not enough gas to deposit code" should {
      ...
      "not modify world state except for the creator's nonce" in {
        val creatorsAccount = context.world.getGuaranteedAccount(fxt.creatorAddr)
        val expectedWorld =
          context.world.saveAccount(fxt.creatorAddr, creatorsAccount.copy(nonce = creatorsAccount.nonce.increaseOne))
        result.world shouldEqual expectedWorld
      }
    ...
    "account with non-zero nonce already exists" should {
      ...
      "fail to create contract" in {
        val accountNonZeroNonce = Account(nonce = Nonce(1))
    ...
    "not deploy contract when initCode starts with ef" in {
      val dataTable =
        Table[String, UInt256, BigInt](
          ("callData", "expectedResult", "gasUsed"),
          ("60ef60005360016000f3", UInt256.Zero, 108053),
          ("60ef60005360026000f3", UInt256.Zero, 108053),
          ("60ef60005360036000f3", UInt256.Zero, 108053),
          ("60ef60005360206000f3", UInt256.Zero, 108053)
        )
      forAll(dataTable) { (callData, expectedResult, gasUsed) =>
        val callBytes = Hex.decodeUnsafe(callData)
        val result    = CreateResult(createCode = callBytes, opcode = opcode)
        result.returnValue shouldBe expectedResult
        result.stateOut.gasUsed shouldBe gasUsed
      }
    }

    "deploy contract when initCode does not start with ef" in {
      val callData = Hex.decodeUnsafe("60fe60005360016000f3")
      val result   = CreateResult(createCode = callData, opcode = opcode)
      result.returnValue should not be UInt256.Zero
    }
  }
    ...
    "returns correct address and spends correct amount of gas (examples from https://eips.ethereum.org/EIPS/eip-1014)" in {
      ...
      forAll(testTable) { (address, salt, init_code, gas, resultAddress) =>
        ...
        val result = CreateResult(
          ...
          opcode = CREATE2,
          createCode = Hex.decodeUnsafe(init_code),
          ownerAddress = add,
          salt = UInt256(Hex.decodeUnsafe(salt))
        )
