...
class StackSpec extends AnyFunSuite with Matchers with ScalaCheckPropertyChecks {
  ...
  test("pop(1) is sound with pop") {
    forAll(Generators.getStackGen(minElems = 1)) { stack =>
      stack.pop(1) match {
        case (Seq(elem), stack1) => (elem, stack1) shouldEqual stack.pop
        case (other, _)          => fail(s"pop(1) should return one element but $other was returned")
      }

    }
  }

  test("pop(1) is sound with pop for empty stack") {
