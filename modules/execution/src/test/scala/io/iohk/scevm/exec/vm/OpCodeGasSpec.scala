...
import io.iohk.scevm.domain.{Account, Address, UInt256}
...
class OpCodeGasSpec extends AnyFunSuite with OpCodeTesting with Matchers with ScalaCheckPropertyChecks {
  ...
  test(EXP) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      val ((_, m: UInt256), _) = stateIn.stack.pop2
  ...
  test(CALLDATACOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      val ((offset, _, size), _) = stateIn.stack.pop3
      val memCost                = config.calcMemCost(stateIn.memory.size, offset, size)
  ...
  test(EXTCODECOPY) { op =>
    ...
    forAll(stateGen) { stateIn =>
      ...
      val ((_, offset, _, size), _) = stateIn.stack.pop4
      val memCost                   = config.calcMemCost(stateIn.memory.size, offset, size)
      val copyCost                  = G_copy * wordsForBytes(size)
  ...
  test(SSTORE) { op =>
    ...
    // Constantinople + Istanbul tested in SSTOREOpCodeGasPostConstantinopleSpec
    ...
    forAll(table) { (offset, value, expectedGas, _) =>
      ...
      val stateIn = getProgramStateGen(
        blockNumberGen = Gen.frequency(
          (1, getBlockNumberGen(max = fixtures.ConstantinopleBlockNumber.previous)),
          (1, getBlockNumberGen(fixtures.PetersburgBlockNumber.next, fixtures.IstanbulBlockNumber.previous))
        ),
        evmConfigGen = Gen.const(petersburgConfig),
        isTopHeader = true
  ...
  test("SSTORE should refund 15000 gas on clear before London") {
    val stateGen = getProgramStateGen(
      stackGen = getStackGen(maxElems = 10), // prevent stack overflow
      evmConfigGen = Gen.const(EvmConfig.BerlinConfigBuilder(blockchainConfig)),
      storageGen = Gen.const(MockStorage.fromSeq(Seq(1)))
    )

    forAll(stateGen) { state =>
      // reset storage offset 0
      val stateIn  = state.copy(stack = state.stack.push(Seq[UInt256](0, 0)))
      val stateOut = SSTORE.execute(stateIn)

      stateOut.gasRefund shouldBe 15000
    }
  }

  test("SSTORE should refund 4800 gas on clear after London") {
    val stateGen = getProgramStateGen(
      stackGen = getStackGen(maxElems = 10), // prevent stack overflow
      evmConfigGen = Gen.const(EvmConfig.LondonConfigBuilder(blockchainConfig)),
      storageGen = Gen.const(MockStorage.fromSeq(Seq(1)))
    )
    ...
    forAll(stateGen) { state =>
      ...
      stateOut.gasRefund shouldBe 4800
    }
  }
  ...
  test(logOps: _*) { op =>
    val table = Table[UInt256, BigInt](
      ("size", "expectedGas"),
      (0, G_log + G_logtopic * op.topicNb),
      (13, G_log + G_logtopic * op.topicNb + G_logdata * 13)
    )
    ...
    forAll(stateGen) { stateIn =>
      ...
      val (Seq(offset, size, _*), _) = stateIn.stack.pop(op.delta): @nowarn("msg=exhaustive")
      val memCost                    = config.calcMemCost(stateIn.memory.size, offset, size)
      val logCost                    = G_logdata * size + op.topicNb * G_logtopic
  ...
  test(SELFDESTRUCT) { op =>
    val stateGen = getProgramStateGen(
      ...
      evmConfigGen = Gen.const(EvmConfig.PostEIP160ConfigBuilder(blockchainConfig))
    )
  ...
  test("SELFDESTRUCT should not refund gas after London") {
    val stateGen = getProgramStateGen(
      stackGen = getStackGen(elems = 1),
      evmConfigGen = Gen.const(EvmConfig.LondonConfigBuilder(blockchainConfig))
    )
    ...
    forAll(stateGen) { stateIn =>
      val stateOut = SELFDESTRUCT.execute(stateIn)
      stateOut.gasRefund shouldEqual 0
    }
  }
