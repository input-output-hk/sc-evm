...
import io.iohk.scevm.mpt.{BranchNode, ExtensionNode, HashNode, LeafNode, MptNode, Node}

class NodeCapper(withUpdates: Boolean) {
  private var nodesToUpdate = List.empty[(Node.Hash, Node.Encoded)]
  ...
  def getNodesToUpdate: List[(Node.Hash, Node.Encoded)] = nodesToUpdate
}
