...
import io.iohk.scevm.mpt.visitors.{MptVisitor, NodeCapper, _}
...
object MptTraversals {
  ...
  def decodeNode(nodeEncoded: Node.Encoded): MptNode =
    parseMpt(decodeNodeRlp(nodeEncoded))

  def decodeNodeRlp(nodeEncoded: Node.Encoded): RLPEncodeable =
