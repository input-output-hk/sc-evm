...
/** Immutable implementation of [[io.iohk.ethereum.common.SimpleMap]] that
  * holds changes made to the inner SimpleMap until `persist()` is called.
  *
  * If the inner SimpleMap is mutable, `persist()` is the only side effect.
  *
