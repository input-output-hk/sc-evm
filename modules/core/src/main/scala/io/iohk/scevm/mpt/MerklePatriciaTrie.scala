...
object MerklePatriciaTrie {
  ...
  val EmptyEncoded: Array[Byte]  = rlp.encode(Array.emptyByteArray)
  val EmptyRootHash: Array[Byte] = Node.hashFn(EmptyEncoded)

  final private case class NodeInsertResult(newNode: MptNode, toDeleteFromStorage: List[MptNode] = Nil)

  final private case class NodeRemoveResult(
      hasChanged: Boolean,
...
class MerklePatriciaTrie[K, V] private (private[mpt] val rootNode: Option[MptNode], val mptStorage: MptStorage)(implicit
    kSerializer: ByteArrayEncoder[K],
    vSerializer: ByteArraySerializable[V]
  ...
  /** Get the value associated with the key passed, if there exists one.
    ...
    * @throws io.iohk.scevm.mpt.MerklePatriciaTrie.MPTException if there is any inconsistency in how the trie is build.
    */
  ...
  private def pathTraverse[T](acc: T, searchKey: Array[Byte])(op: (T, Option[MptNode]) => T): Option[T] = {
    ...
    def pathTraverse(acc: T, node: MptNode, searchKey: Array[Byte], op: (T, Option[MptNode]) => T): Option[T] =
      node match {
        ...
        case HashNode(bytes) =>
          pathTraverse(acc, getFromHash(bytes, mptStorage), searchKey, op)
  ...
  private def getFromHash(nodeId: Array[Byte], source: MptStorage): MptNode = {
    val nodeEncoded = source.getNodeOrFail(nodeId).encode
    MptTraversals
  ...
  override def put(key: K, value: V): MerklePatriciaTrie[K, V] = {
    ...
    rootNode
      .map { root =>
        ...
        val newRootNode                                         = mptStorage.updateNodesInStorage(newRoot = Some(newRoot), toRemove = nodesToRemoveFromStorage)
        new MerklePatriciaTrie(newRootNode, mptStorage)(kSerializer, vSerializer)
      }
      .getOrElse {
        val newRoot     = LeafNode(ByteString(keyNibbles), ByteString(vSerializer.toBytes(value)))
        val newRootNode = mptStorage.updateNodesInStorage(Some(newRoot), Nil)
        new MerklePatriciaTrie(newRootNode, mptStorage)
      }
  }
      ...
      .map { root =>
        ...
        remove(root, keyNibbles) match {
          case NodeRemoveResult(true, Some(newRoot), nodesToRemoveFromStorage) =>
            val newRootNode =
              mptStorage.updateNodesInStorage(newRoot = Some(newRoot), toRemove = nodesToRemoveFromStorage)
            new MerklePatriciaTrie(newRootNode, mptStorage)(kSerializer, vSerializer)
          case NodeRemoveResult(true, None, nodesToRemoveFromStorage) =>
            mptStorage.updateNodesInStorage(newRoot = None, toRemove = nodesToRemoveFromStorage)
            new MerklePatriciaTrie(None, mptStorage)(kSerializer, vSerializer)
  ...
  private def get(node: MptNode, searchKey: Array[Byte]): Option[Array[Byte]] = node match {
    ...
    case HashNode(bytes) =>
      get(mptStorage.getNodeOrFail(bytes), searchKey)
  ...
  private def put(node: MptNode, searchKey: Array[Byte], value: Array[Byte]): NodeInsertResult = node match {
    ...
    case HashNode(bytes) =>
      put(mptStorage.getNodeOrFail(bytes), searchKey, value)
  ...
  private def putInLeafNode(node: LeafNode, searchKey: Array[Byte], value: Array[Byte]): NodeInsertResult = {
    ...
    ByteUtils.matchingLength(existingKey.toArray[Byte], searchKey) match {
      ...
      case 0 =>
        ...
        val NodeInsertResult(newBranchNode, toDeleteFromStorage) = put(temporalBranchNode, searchKey, value)
        NodeInsertResult(
      ...
      case ml =>
        ...
        val NodeInsertResult(newBranchNode, toDeleteFromStorage) = put(temporalNode, searchKeySuffix, value)
  ...
  private def putInExtensionNode(
    ...
    ByteUtils.matchingLength(sharedKey.toArray[Byte], searchKey) match {
      case 0 =>
      ...
      case ml if ml == sharedKey.length =>
        ...
        val NodeInsertResult(newChild, toDeleteFromStorage) = put(extensionNode.next, searchKey.drop(ml), value)
      ...
      case ml =>
        ...
        val NodeInsertResult(newBranchNode, toDeleteFromStorage) = put(temporalExtensionNode, searchKey.drop(ml), value)
  ...
  private def remove(node: MptNode, searchKey: Array[Byte]): NodeRemoveResult = node match {
    ...
    case HashNode(bytes) =>
      remove(mptStorage.getNodeOrFail(bytes), searchKey)
  ...
  private def removeFromExtensionNode(extensionNode: ExtensionNode, searchKey: Array[Byte]): NodeRemoveResult = {
    ...
    val cp                                   = ByteUtils.matchingLength(sharedKey.toArray[Byte], searchKey)
  ...
  /** Given a node which may be in an invalid state, fix it such that it is then in a valid state. Invalid state means:
    ...
    * @param mptStorage  to obtain the nodes referenced in the node that may be in an invalid state.
    * @param notStoredYet to obtain the nodes referenced in the node that may be in an invalid state,
    *                     if they were not yet inserted into the mptStorage.
    * @return fixed node.
    * @throws io.iohk.ethereum.mpt.MerklePatriciaTrie.MPTException if there is any inconsistency in how the trie is build.
    */
  @tailrec
    ...
    case extensionNode @ ExtensionNode(sharedKey, _, _, _, _) =>
      val nextNode = extensionNode.next match {
        case HashNode(nextHash) =>
          ...
          mptStorage.getNodeOrFail(nextHash) // We search for the node in the db
