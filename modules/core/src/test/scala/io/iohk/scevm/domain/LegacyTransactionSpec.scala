...
class LegacyTransactionSpec extends AnyWordSpec with SignedTransactionBehavior with ScalaCheckPropertyChecks {
  ...
  "Signed LegacyTransaction" should behave.like(SignedTransactionBehavior(legacyTransactionGen, allowedPointSigns))

  "LegacyTransaction" should {
    "be properly recover the sender from rlp encoded values" in {

      implicit val chainId = ChainId(1)
      ...
      forAll(testValues) { (binaryRLP: String, expectedSender: String) =>
        val decodedSignedTransaction = Hex.decodeAsArrayUnsafe(binaryRLP).toSignedTransaction
    ...
    "respect EIP155 signature  example" in {
      ...
      val signingData = SignedTransaction.bytesToSign(legacyTransaction, Some(ChainId(1)))

      Hex.toHexString(signingData) shouldEqual expectedSigningHash

      val senderKeyPair     = ECDSA.PrivateKey.fromHexUnsafe(privateKey)
      val signedTransaction = SignedTransaction.sign(legacyTransaction, senderKeyPair, Some(ChainId(1)))
