...
import io.iohk.scevm.mpt.MerklePatriciaTrie.MPTException
import io.iohk.scevm.mpt.ProofVerifyResult.ValidProof
...
class MerklePatriciaTrieSuite extends AnyFunSuite with ScalaCheckPropertyChecks {

  import io.iohk.scevm.testing.Generators._

  val emptyEphemNodeStorage: MptStorage = new ArchiveMptStorage(new EphemeralMptStorage)
  ...
  test("Remove of a trie with an extension whose next is not on source") {
    ...
    val wrongSource       = new EphemeralMptStorage
    val rootValue         = trie.mptStorage.getNodeOrFail(trie.getRootHash).cachedRlpEncoded.get
    wrongSource.update(
      Nil,
      Seq(ByteString(trie.getRootHash) -> rootValue)
    )
    val trieAfterDelete = Try {
      val trieWithWrongSource =
        MerklePatriciaTrie[Array[Byte], Array[Byte]](trie.getRootHash, wrongSource)
  ...
  test("EthereumJ compatibility - Empty Trie") {
    assert(Hex.toHexString(emptyMpt.getRootHash) == emptyStorageRoot.toHex)
  }
