...
import io.iohk.scevm.network.rlpx.{AuthHandshakeSuccess, AuthHandshaker, AuthResponseMessage, Secrets}
...
class AuthHandshakerSpec extends AnyFlatSpec with Matchers {
  ...
  "AuthHandshaker" should "handle init response" in {
    ...
    val AuthHandshakeSuccess(secrets: Secrets, _) =
      authHandshaker.handleResponseMessage(ByteString(encryptedResponse)): @nowarn("msg=exhaustive")
  ...
  it should "handle both incoming packet and a response" in {
    ...
    val AuthHandshakeSuccess(thisSecrets: Secrets, _) =
      thisHandshakerInitiated.handleResponseMessageV4(responsePacket): @nowarn("msg=exhaustive")
