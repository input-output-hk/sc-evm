...
import io.iohk.scevm.domain.{Account, Address, Nonce, Transaction, UInt256}
...
object EvmTestEnv {
  val ContractsDir = "tooling/evmTest/target/contracts"
}

// scalastyle:off magic.number
trait EvmTestEnv extends ScalaFutures with IOSupport with NormalPatience {

  import EvmTestEnv._

  val config: EvmConfig = EvmConfig.ConstantinopleConfigBuilder(fixtures.blockchainConfig)
  ...
  val defaultContractCreator: Address = createAccount(balance = 1000)
  val defaultSender: Address          = createAccount(balance = 1000)
  ...
  def deployContract(
      ...
      gasPrice: BigInt = 1,
      constructorArgs: Seq[Any] = Nil,
      contractsDir: String = ContractsDir
  ): (ProgramResult[MockWorldState, MockStorage], Contract) = {
    ...
    val tx: Transaction = MockVmInput.mockTransaction(payload, value, gasLimit, gasPrice)
    val blockContext    = MockVmInput.blockContext

    val context =
      ProgramContext[MockWorldState, MockStorage](tx, blockContext, creatorAddress, worldAfterNonceIncrease, config)
    ...
    internalWorld = result.world
      .saveAccount(
        contractAddress,
        Account(Nonce.Zero, UInt256(value), Account.EmptyStorageRootHash, kec256(result.returnData))
      )
  ...
  private def parseArg(arg: Any): ByteString = arg match {
    ...
    case _              => throw new RuntimeException("Invalid call argument")
  }
  ...
  class ContractMethodCall(contract: Contract, callData: ByteString) {
    def callWithCustomMV(
        vm: EVM[MockWorldState, MockStorage],
        value: BigInt = 0,
        gasLimit: BigInt = BigInt(2).pow(256) - 1,
        gasPrice: BigInt = 1,
        sender: Address = defaultSender
    ): (ProgramContext[MockWorldState, MockStorage], ProgramResult[MockWorldState, MockStorage]) = {
      val transaction: Transaction =
        MockVmInput.mockTransaction(callData, value, gasLimit, gasPrice, Some(contract.address))
      val blockHeader = MockVmInput.blockContext
      val pc          = ProgramContext[MockWorldState, MockStorage](transaction, blockHeader, sender, world, config)

      val res = vm.run(pc).ioValue
    ...
    def call(
      ...
      callWithCustomMV(vm, value, gasLimit, gasPrice, sender)._2
  }
