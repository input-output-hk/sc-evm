...
class RLPSuite extends AnyFunSuite with ScalaCheckPropertyChecks with ScalaCheckDrivenPropertyChecks {
  ...
  test("Long Encoding") {
    val values = Table("value", -1L, -1024L, 1L, 1024L, 0L, Long.MinValue, Long.MaxValue)

    def testLongEncoding = { (aLong: Long) =>
    ...
    forAll(values)(testLongEncoding)

    forAll(Gen.long)(testLongEncoding)
  }
  ...
  test("Byte Array Encoding") {
    ...
    val byteArray: Array[Byte] = Hex.decodeAsArrayUnsafe(byteArr)
  ...
  test("Partial Data Parse Test") {
    ...
    val data              = Hex.decodeAsArrayUnsafe(hex)
