...
object RLP {
  ...
  /** This function converts a long value to a big endian byte array of minimal length
    *
    * @param v value to encode
    * @return encoded bytes
    */
  private[rlp] def longToBigEndianMinLength(v: Long): Array[Byte] =
    Array.from(0.to(7).reverse.view.map(shift => (v >> (shift * 8)).toByte).dropWhile(_ == 0))
  ...
  /** This function converts from a big endian byte array of minimal length to a long value
    *
    * @param bytes encoded bytes
    * @return Int value
    * @throws RLPException If the value cannot be converted to a valid int
    */
  private[rlp] def bigEndianMinLengthToLong(bytes: Array[Byte]): Long =
    if (bytes.length <= 8) {
      bytes.foldLeft(0L)((value, byte) => (value << 8) + (byte & 0xff))
    } else {
      throw RLPException("Bytes don't represent a long")
    }
...
final private case class ItemBounds(start: Int, end: Int, isList: Boolean, isEmpty: Boolean = false)
