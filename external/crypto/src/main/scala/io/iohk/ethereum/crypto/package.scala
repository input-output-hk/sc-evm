...
class Crypto[F[_]: Sync](secureRandom: SecureRandom) {

  def generateKeyPair(): F[(ECDSA.PrivateKey, ECDSA.PublicKey)] = Sync[F].delay {
    ECDSA.generateKeyPair(secureRandom)
  }
}

object Crypto {

  def apply[F[_]](implicit ev: Crypto[F]): Crypto[F] = ev
}

package object crypto {

  type ECDSA        = ECDSA.type
  type AnySignature = AbstractSignature[_ <: AbstractPublicKey, _ <: AbstractSignature[_, _]]
